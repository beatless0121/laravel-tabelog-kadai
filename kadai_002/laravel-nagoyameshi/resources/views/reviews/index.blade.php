    ...
    <div class="container nagoyameshi-container pb-5">
        <div class="row justify-content-center">
            <div class="col-xxl-6 col-xl-7 col-lg-8 col-md-10">
                ...
                @foreach ($reviews as $review)
                    <div class="card mb-3">
                        <div class="card-header d-flex justify-content-between">
                            <div>
                                {{ $review->member->name }}さん
                            </div>
                            @if ($review->member_id === Auth::id())
                                <div>
                                    <a href="{{ route('shops.reviews.edit', [$shop, $review]) }}" class="me-2">編集</a>
                ...
                @if (!Auth::user()->subscribed('premium_plan') && $shop->reviews()->count() > 3)
                ...
                @if (Auth::user()->subscribed('premium_plan') && $shop->reviews()->where('member_id', Auth::id())->doesntExist())
                    <div class="text-center mt-3">
                        <a href="{{ route('shops.reviews.create', $shop) }}" class="btn text-white shadow-sm w-50 nagoyameshi-btn">レビューを投稿する</a>
                    </div>
                @endif
            </div>
        </div>
